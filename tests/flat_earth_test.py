...
def test1():
    ...
    scints_1_weak = np.array([ scint_index_DL_PR_weak(wavelen, zenang, altitude, ris_model_1) for zenang in zeniths_rad ]) # type: ignore
    scints_2_weak = np.array([ scint_index_DL_PR_weak(wavelen, zenang, altitude, ris_model_2) for zenang in zeniths_rad ]) # type: ignore
    scints_1_modstr = np.array([ scint_index_DL_PR_general(wavelen, zenang, altitude, ris_model_1) for zenang in zeniths_rad ]) # type: ignore
    scints_2_modstr = np.array([ scint_index_DL_PR_general(wavelen, zenang, altitude, ris_model_2) for zenang in zeniths_rad ]) # type: ignore
...
def test2():
    ...
    scints_za1 = [scint_index_DL_AA_general(wavelen, zangle1, altitude, hufnagel_valley_model, diam) for diam in diameters] # type: ignore
    scints_za2 = [scint_index_DL_AA_general(wavelen, zangle2, altitude, hufnagel_valley_model, diam) for diam in diameters] # type: ignore
    ...
    # ax.set_ylim(bottom=-0.19, top=0.26)
...
from src.optical_turbulence.flat_earth import _total_beam_wander_variance_UL_gaussian
...
def test3():
    ...
    link_len_1 = link_distance / np.cos(zangle1)
    link_len_2 = link_distance / np.cos(zangle2)
    ...
    for i, bradius in enumerate(beam_radii):
        rms_angular_1[i] = _total_beam_wander_variance_UL_gaussian(zenith_angle=zangle1, beam_radius=bradius, **common_parameters)
        rms_angular_2[i] = _total_beam_wander_variance_UL_gaussian(zenith_angle=zangle2, beam_radius=bradius, **common_parameters)
...
def test4():
    ...
    zangle = 0 * (np.pi / 180)
    pfront_radius = np.inf
    link_length = ( np.max(altitude) - np.min(altitude) ) / np.cos(zangle) # [m]
